...
| 7 | `7_reserved` | 用途未定の予約レイヤー。全キー `&trans` | ―（未配線） |
| 8 | `8_reserved` | 用途未定の予約レイヤー。全キー `&trans` | ―（未配線） |
| 9 | `9_reserved` | 用途未定の予約レイヤー。全キー `&trans` | ―（未配線） |
...
用途未定の予約レイヤー。**現状すべて `&trans`**。`4_func_win`・`5_mouse` と同様に
**到達手段が定義されていない**（`&lt 7` / `&mo 7` / `&to 7` がキーマップ内に存在しない）。

## レイヤー 8: 8_reserved

用途未定の予約レイヤー。**現状すべて `&trans`**。`7_reserved` と同様に
**到達手段が定義されていない**（`&lt 8` / `&mo 8` / `&to 8` がキーマップ内に存在しない）。

## レイヤー 9: 9_reserved

用途未定の予約レイヤー。**現状すべて `&trans`**。`7_reserved` と同様に
**到達手段が定義されていない**（`&lt 9` / `&mo 9` / `&to 9` がキーマップ内に存在しない）。
...
4. `3_function` のコピペショートカットはCmd固定（Mac向け）。Windows接続時の差分は
   `4_func_win` に未実装。
5. `7_reserved`・`8_reserved`・`9_reserved` は用途・到達手段とも未定の予約レイヤー。
