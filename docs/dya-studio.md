...
**症状**: 速くスクロールすると、ときどき方向が逆になる。

**原因**: `zmk-module-runtime-input-processor` の `scale_val()` が、乗算結果を
**int16_t に切り詰めていた**。

```c
int16_t value_mul = event->value * (int16_t)mul;   /* event->value は int32_t */
```

**「倍率が縮小（0.02x など）だから安全」とは言えない。** DYA Studio は要求された倍率を
**1000分の1の分数**にして gcd で約分するだけ（`dya-studio` の `TrackballPage.tsx` の
`scalingValueToFraction()`、`SCALING_PRECISION = 1000`）。割り切れない値では**3桁の乗数が
そのまま残る**。スライダーは対数刻みで端数の値になり、表示は `toFixed(2)` で丸められるため、
**画面上は区別できない**。

```
表示 | 乗数 | 除数 | 2047×乗数 | 判定
0.02 |   17 | 1000 |   34799 | 溢れる
0.02 |   19 | 1000 |   38893 | 溢れる
0.02 |   21 | 1000 |   42987 | 溢れる
0.02 |   23 | 1000 |   47081 | 溢れる
```

センサーが 12bit 上限の 2047 カウントを出すと、乗数21なら `2047 × 21 = 42987` が
int16 を超えて **-22549** になり、÷1000 で **-22**。前進スクロールの途中に逆スクロールが
混ざる。**スライダー101段のうち75段**がこの条件に該当する。

速い動きでしか出ないのは、1サンプルが `32767 ÷ 乗数` カウントを超える必要があるため。

`config/west.yml` が指すフォークの `fix/scale-int16-overflow` で int32 演算に修正済み。

#### 切り分けの記録（2026-09）

ブラウザで実際にホストが受け取っている値を観測したのが決め手になった。
...
```
1 -0  (22回)          ← ゆっくり。センサー値 約50 → 50×21÷1000 = 1
-51.189  -4.000       ← 異常。ファームは -22 を送り、macOS が増幅
...
24, 22, 23, 44 -0     ← 速く
-521.432  -14.619     ← 異常。同じ -22 でも加速が強いぶん大きく出る
```

**macOS の加速カーブは非線形なので、ブラウザの値からファーム側の実値は逆算できない。**
大きさを手がかりにした推論は当てにならず、実際これで何度も見立てを誤った。

**効かなかった対処**（いずれも別の故障モードへの対処だったため）:

| 対処 | 結果 |
|---|---|
| `force_awake` ON / レストモードのサンプル周期短縮 / ダウンシフト延長 | 効果なし |
| ドライバに OVF 検出を追加（下記） | 効果なし。OVF はセンサー内部の蓄積あふれで別物 |
| 軸スナップ OFF | 効果なし |
| SPI クロック 2MHz → 1MHz | 効果なし。ただしデータシート上 2MHz は**最大値**なので下げたまま残す |

**切り分けが決まった観測**: ポインタ移動では起きず**スクロールだけ**で起きること。
センサーとドライバは両経路で共通なので、これでセンサー側が除外され、
スクロール専用チェーン（= scroll プロセッサー）に絞り込めた。

#### OVF 検出について（残してあるが今回の原因ではない）

センサーのデルタレジスタは 12bit 二の補数（±2047 カウント）。読み出し間隔中に
これを超えるとカウンタが一周して符号が反転する。データシート
（`PMS0003-PMW3610DM-SUDU-DS-R2.4` の MOTION レジスタ）にはこう書かれている。
```

```
20   -0          ← 通常
23   -0
-534.398  -496.634    ← 異常。両軸そろって約 -500
21   -0
-522.668  -498.697    ← 3〜5イベントに1回の頻度
```

**この観測から分かること**:

- 符号は**確かに反転している**。macOS の加速は大きさを増幅するが符号は変えないので、
  負の値はデバイス側から来ている
- しかし**実際の動きではない**。縦スクロール中で横にはほぼ動かしていない
  （通常時 `deltaX = -0` がその証拠）のに、異常時は X が Y と同程度の大きさになる
- **12bit ラップでもない**。ラップには元の値が 2047 を超える必要があり、
  ほぼ静止している X では起こり得ない
- バースト読み出しが同じバイトの繰り返しになると、復号式の構造上
  `x = X_L + ((XY_H & 0xF0) << 4)` と `y = Y_L + ((XY_H & 0x0F) << 8)` が
  **同じ値になる**。観測値の Y/X 比が 1.02〜1.08 に収まるのは、macOS が縦横で
  別の加速カーブを使うためと説明がつく

→ **転送データの破損**と判断。`spi-max-frequency` を 2MHz から 1MHz へ半減させた。
データシートの推奨動作条件では `fSCLK = 2MHz` が**最大値**で、上流の作例は上限
ちょうどを使っており、分割キーボードの配線長では余裕が無かった。
...
|---|---|
| `force_awake` ON / レストモードのサンプル周期短縮 / ダウンシフト延長 | 効果なし |
| ドライバに OVF 検出を追加（下記） | 効果なし。OVF はセンサー内部の蓄積あふれで、SPI 転送の破損とは別物 |
| 軸スナップ OFF | 効果なし |
| `scale_val` の int16 溢れ | 該当せず。スケーリングは 0.02x（縮小）で溢れない |
...
> If one of the 12 bits motion registers overflows, then absolute path data is lost
> and the OVF bit is set. **Once OVF bit set, Sensor will stop accumulating motion data.**

つまりオーバーフロー後はラップした値のまま凍結される。MOTION レジスタ（0x02）の
ビット構成は次のとおりで、**OVF = BIT(4)**。

```
Bit    7    6         5         4    3         2          1         0
Field  MOT  Reserved  Reserved  OVF  LP_VALID  LSR_FAULT  Reserved  RST_FLAG
```

上流ドライバは burst の `buf[0]`（= MOTION レジスタ）を読んでいながら**一度も参照せず**、
ラップした値をそのまま報告していた。`config/west.yml` が指すフォークの
`fix/motion-overflow` で、OVF が立っていたらそのサンプルを捨てるようにしてある。

**オーバーフローは「異常に速い操作」を必要としない。** 2047 カウント = CPI 600 で
ボール表面 3.41 インチ。読み出しが遅れれば簡単に到達する。
...
> **CPI を上げるとこの問題は悪化する。** 低速時の追従改善のために CPI を上げると、
> 高速時の反転が起きやすくなる。両立させるには CPI ではなく、レストモードの
> サンプル周期や後段のスケールで調整する。

読み出しが遅れる原因は 2 つある。レストモードの長いサンプル周期
（REST3 は 500ms）と、**モーション割り込みの処理が他の仕事で遅延すること**。
本リポジトリは BLE 周りの CPU 負荷問題を抱えている（`CLine46_R.conf` の
`CONFIG_BT_CTLR_ASSERT_OVERHEAD_START` のコメント参照）ため後者が効きやすく、
`force_awake` やレスト時間の調整では解消しなかった。

### この件で得た教訓

**表示されている倍率から内部表現を推測しない。** 「0.02x は縮小だから乗数は小さいはず」
という推測で、真犯人の `scale_val()` を一度は「今回の症状には無関係」と除外してしまった。
実際には表示 0.02x の裏に乗数 21 が隠れていた。設定値を疑うときは
**UI の表示ではなく、UI 側の変換コードか実際に送られる値を確認する**。

**macOS の加速カーブは非線形なので、ブラウザ側の値の大きさからファーム側の実値は
逆算できない。** 大きさを根拠にした推論（「2047カウントに相当するから12bitラップだ」など）
は当てにならない。**符号**と**発生条件**だけが信頼できる手がかりだった。

**効いた切り分けは「ポインタでは起きずスクロールだけで起きる」という一点。**
センサーとドライバは両経路で共通なので、これだけでセンサー側が丸ごと除外でき、
スクロール専用チェーンに絞り込めた。ここに早く気づいていれば、レストモード・OVF・
SPI クロックへの4回の回り道は避けられた。
