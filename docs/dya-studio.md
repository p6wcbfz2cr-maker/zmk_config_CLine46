...
### ⚠️ スクロール速度は「乗数」ではなく「除数」で調整する

`zmk-module-runtime-input-processor` の `scale_val()` に int16 切り詰めがある。

```c
int16_t value_mul = event->value * (int16_t)mul;   /* event->value は int32_t */
```

センサーの1サンプルは最大 ±2047 なので、**乗数を 17 以上にすると
`2047 × 17 = 34799` が int16 を超えて符号が反転する**（= スクロールが逆走する）。

現在の設定（乗数 1 / 除数 8）では発動しない。速くしたいときは
**乗数を上げず、除数を下げること**（例: 8 → 4 で 2 倍）。除数側にこの制限はない。

このバグ自体は未修正（上流のまま）。修正するにはこのモジュールもフォークが要るため、
追従対象を増やさない判断で見送っている。
