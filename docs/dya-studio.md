...
**原因**: **Mac Mouse Fix**（macOS のマウス/スクロール系ユーティリティ）。
止めたら再現しなくなった。**ファームウェアは無関係だった。**

この種のツールはスクロールイベントを横取りして独自の慣性・加速処理をかける。
ZMK 側は正しい符号の値を送っており、反転はホスト側で作られていた。

#### 切り分けの記録（2026-09）

**遠回りした。** 原因がファームにあると思い込み、センサー側の対処を4回、
数値処理の修正を1回行ったが、**どれも症状を一切変えなかった**。

| 試した対処 | 結果 |
|---|---|
| `force_awake` ON / レストモードのサンプル周期短縮 / ダウンシフト延長 | 効果なし |
| ドライバに OVF 検出を追加 | 効果なし |
| 軸スナップ OFF | 効果なし |
| SPI クロック 2MHz → 1MHz | 効果なし |
| `scale_val` の int16 溢れを int32 化 | 効果なし |

**効いた切り分け**:

1. **ブラウザで実際の値を観測する**（ホスト側で何が起きているかを見る）
   ...
   20   -0          ← 通常
   23   -0
   -534.398  -496.634    ← 異常。約 -500
   21   -0
   ```

2. **別デバイスで同じログを取る** → 異常が出ない = macOS が無条件に生成しているのではない
3. **ポインタ経路を同じボール速度で試す** → 異常が出ない = センサー・ドライバは健全
4. **常駐ユーティリティを止める** ← **これが正解だった**

#### 見落としの教訓

**「ファームを変えても症状の大きさが変わらない」こと自体が最大の手がかりだった。**
特に `scale_val` の int32 化は演算経路そのものを変えたのに、異常値は約 -500 のまま
だった。**ファームが値を作っているなら大きさが動くはず**で、動かない以上ファームは
関与していないと読めた。5回変更して初めて気づいたのでは遅い。

**ホスト側の常駐ソフトは、ブラウザで反転を観測した時点で真っ先に確認すべきだった。**
デバイス側のコードを読み進めれば答えが出るという前提を持ち続けたのが誤り。

**表示されている倍率から内部表現を推測しない。** 「0.02x は縮小だから乗数は小さい」
という推測で `scale_val` を一度除外したが、実際は表示 0.02x の裏に乗数 21 が隠れていた
（今回の原因ではなかったが、バグ自体は実在する。下記）。

**macOS の加速カーブは非線形なので、ブラウザ側の値の大きさからファーム側の実値は
逆算できない。** 信頼できたのは**符号**と**発生条件**だけだった。

#### 残した修正（今回の原因ではないが、それ自体は正しい）

**1. `scale_val()` の int32 化**（`config/west.yml` のフォーク）

`zmk-module-runtime-input-processor` が乗算結果を int16_t に切り詰めていた。

```c
int16_t value_mul = event->value * (int16_t)mul;   /* event->value は int32_t */
```

DYA Studio は要求倍率を**1000分の1の分数**にして gcd で約分するだけ
（`dya-studio` の `TrackballPage.tsx` の `scalingValueToFraction()`）。割り切れない値では
**3桁の乗数がそのまま残る**。スライダーは対数刻みで端数になり、表示は `toFixed(2)` で
丸められるため**画面上は危険な段か判別できない**。

```
表示 | 乗数 | 除数 | 2047×乗数 | 判定
0.02 |   21 | 1000 |   42987 | int16 を超えて -22549 になる
```

**スライダー101段のうち75段**が、センサーの 12bit 上限 2047 で溢れる乗数を持つ。
実際に踏む可能性が高いので修正を残した。

**2. `spi-max-frequency` を 1MHz に**（`CLine46_R.overlay`）

データシートの推奨動作条件では `fSCLK = 2MHz` が**最大値**。上流の作例は上限ちょうどを
使っており、分割キーボードの配線長では余裕が無い。コストゼロなので下げたまま残す。

**戻した修正**: ドライバへの OVF 検出。データシート準拠の正しい防御コードではあるが、
実機で発動した証拠が無く、フォーク1個分の追従コストに見合わないため上流に戻した。
### 追い込みの手順
...
| **速くスクロールすると方向が逆になる** | まず **Mac Mouse Fix などマウス/スクロール系の常駐ユーティリティを止めて試す**。過去にこれが原因だった。ファーム側を疑う前にホスト側を確認すること。詳細は上記「スクロール方向の反転」 |
